...
bazel run @rules_monorepo//tools/oci -- push --bazel ./tools/bazel \
  --image worker --tag "$GIT_SHA" --tag latest --jobs 4
```

`--scope` limits discovery to a query scope. `--image` accepts a logical name,
the generated target name, or a full label and may be repeated. Pushes use at
most `--jobs` concurrent Bazel processes. Use `--dry-run` to validate selection
and command construction without loading or pushing images.
